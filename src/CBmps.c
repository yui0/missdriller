...
{
	CBmpsLoadWzNum(this,filename,this->index);
	//JYCET correction transparence
	CBmpsSetTransparent(this, 255, 0, 255);
	return (0);
}
...
画面の初期化、スクリーンを返す
*/
SDL_Surface* CScreenInitDefault(int w, int h)
{
	int flags=SDL_SWSURFACE;
	return CScreenInit(w, h, 16, flags);
}

SDL_Surface* CScreenInitDefaultHW(int w, int h)
{
...
//	int flags=SDL_HWSURFACE|SDL_HWPALETTE;
	...
	int flags=SDL_FULLSCREEN|SDL_HWSURFACE|SDL_HWPALETTE;
	return CScreenInit(w, h, 16, flags);
}
...
//		sprintf(filenamebuf,"%s%s",env->bitmapsdir,p);
		...
		if(NULL==p){//if more character exists,doesn't set transparent color
			trans=SDL_MapRGB(this->bmp[index]->format,255,0,255);
